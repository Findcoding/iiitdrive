...
def get_trending_tags(trend_hours=5, trend_values=3) :
	...
	trend_tags = Tag.objects.filter(posts__created_at__lt = time_threshold_t).values('name').annotate(count = models.Count('posts')).order_by('-count')[:trend_values].values_list('name', flat = True)
